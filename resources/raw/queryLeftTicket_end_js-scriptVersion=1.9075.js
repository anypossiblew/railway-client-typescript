...
(function() {
    ...
    var bp = function(cx) {
        var cy, cu = {}, cz, cv = this, ct = false, cr, cw, cs = {
            x: 10,
            y: 66
        }, cq = {
            x: 5,
        ...
        this.init = function() {
            ...
            cz.css(cx, cs.x + "px").css({
                top: cs.y + "px"
            }).on("mouseenter", cv.stop).on("mouseleave", cv.resize).children("a.close").on("click", cv.hidden);
            $("body").append(cz);
            cr = cs.x;
            cw = cs.y;
            cv.resize()
                    ...
                    success: function(cx) {
                        if (cx.status) {
                            ...
                            if (train_tour_flag != null && train_tour_flag == "fc") {
                                ...
                                if (bX(cx.data)) {
                                    cv = "<p class='ad-gt' id='DW_SHOW_STR' data='1'>高铁动卧 夕发朝至 风雨无阻 省时省钱</p>"
                                } else {
